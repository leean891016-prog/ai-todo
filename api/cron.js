// Vercel Cron Function — checks reminders every minute, sends Web Push
  ...
  try {
    ...
    try {
      if (process.env.KV_REST_API_URL) {
        data = await kv.get('data', { type: 'json' }) || { devices: {} };
      }
    ...
    for (const dev of Object.values(data.devices)) {
      if (dev._fired) {
        for (const key of Object.keys(dev._fired)) {
          if (key !== today) { delete dev._fired[key]; changed = true; }
        }
      }
    }
